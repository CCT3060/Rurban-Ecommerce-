...
export async function POST(request: Request) {
  ...
  const categoryName = String(formData.get("categoryName") || "category");
  ...
  const safeSlug = generateSlug(categoryName) || "category";
  const filePath = `categories/${safeSlug}-${Date.now()}.${extension}`;
  ...
  const { data } = admin.storage.from("categories").getPublicUrl(filePath);
