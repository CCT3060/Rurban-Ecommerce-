...
export async function POST(request: Request) {
  ...
  const { data: targetUser } = await admin
    ...
    .select("id, email")
    .eq("id", userId)
    .maybeSingle() as unknown as { data: { id: string; email: string } | null };
  ...
  for (const row of rows) {
    ...
    if (existing) {
      const { error } = await db.from("user_product_prices").update(payload).eq("id", existing.id);
      if (error) { addError((error as { message: string }).message); continue; }
      result.updated++;
    } else {
      const { error } = await db.from("user_product_prices").insert({ user_id: userId, product_id: productId, ...payload });
      if (error) { addError((error as { message: string }).message); continue; }
      result.created++;
    }
  }
