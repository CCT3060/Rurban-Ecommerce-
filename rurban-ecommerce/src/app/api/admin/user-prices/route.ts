...
// ─── GET /api/admin/user-prices ────────────────────────────────────────────
// Returns list of user-product price records with related user and product info.
// Query params: userId, productId, categoryId, status, search, page, limit
export async function GET(request: Request) {
  ...
  const limit = Math.min(10000, Math.max(1, parseInt(searchParams.get("limit") ?? "2000", 10)));
  ...
  // Filter by category or search in JS (Supabase doesn't support deep filtering)
...
// ─── POST /api/admin/user-prices ───────────────────────────────────────────
// Create a new user-product price record.
export async function POST(request: Request) {
  ...
  const { data, error } = await admin
    .from("user_product_prices")
    .insert({ user_id: userId, product_id: productId, custom_price: customPrice, status })
    .select()
    .single();
