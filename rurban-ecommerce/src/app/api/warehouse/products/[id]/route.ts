...
export async function PUT(
  ...
  const normalizedImageUrl = body.image_url !== undefined ? normalizeSupabaseImageUrl(body.image_url) || null : undefined;
  ...
  if (scopedProductError) {
    return NextResponse.json({ error: scopedProductError.message }, { status: 400 });
  }
  ...
  if (normalizedImageUrl) {
    ...
    if (existingPrimary?.id) {
      const { error: imageUpdateError } = await admin.from("product_images").update({ image_url: normalizedImageUrl }).eq("id", existingPrimary.id);
      if (imageUpdateError) return NextResponse.json({ error: imageUpdateError.message }, { status: 400 });
    } else {
      ...
      if (imageInsertError) return NextResponse.json({ error: imageInsertError.message }, { status: 400 });
    }
  }
  ...
  if (body.variants !== undefined) {
    ...
    const { error: deleteVariantsError } = await admin.from("product_variants").delete().eq("product_id", id);
    if (deleteVariantsError) return NextResponse.json({ error: deleteVariantsError.message }, { status: 400 });

    if (normalizedVariants.length > 0) {
      const { error: insertVariantsError } = await admin.from("product_variants").insert(normalizedVariants);
      if (insertVariantsError) return NextResponse.json({ error: insertVariantsError.message }, { status: 400 });
    }
  }
