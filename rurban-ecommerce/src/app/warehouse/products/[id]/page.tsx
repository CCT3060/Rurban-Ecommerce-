...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      setFetching(true);
      try {
        const response = await fetch(`/api/warehouse/products/${id}`, { cache: "no-store" });
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Edit Warehouse Product</h1>
          <p className="text-sm text-muted-foreground">Update warehouse product details and save changes</p>
        </div>
      </div>
